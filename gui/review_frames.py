...
if QMainWindow is not None:
    ...
    class ReviewWidget(QWidget):
        ...
        def source_filter_options(self) -> list[dict[str, str]]:
            ...
            for idx, row in enumerate(self._all_rows):
                ...
                group = groups.setdefault(
                    ...
                    {
                        "count": 0,
                        "name": source_label or (Path(video).name if video else i18n.t("REVIEW_SOURCE_FILTER_UNASSIGNED")),
                        "first_index": idx,
                    },
                )
        ...
        def _row_projection(self, row: dict[str, str]) -> str:
            return self._projection_by_rel.get(self._row_output_key(row), PROJECTION_UNKNOWN)

        def _row_supports_perspective(self, row: dict[str, str]) -> bool:
            return self._row_projection(row) == PROJECTION_EQUIRECTANGULAR

...
_TIPS_JA: dict[str, str] = {
    ...
    "YOLO_EXPAND": "主マスク境界を固定ピクセルで補正します。既定は2px。横ドラッグで調整可能。安全範囲は -16〜32px",
    ...
    "SKY_MIN_SCORE": "Mask2Formerで選択クラスのスコアがこの値以上の画素だけ採用します。0で無効。誤検出が多い場合だけ上げます",
    "SKY_MIN_AREA": "小さな空候補を面積比で除去します。既定は0.05%。細かい誤検出を抑えるための設定です",
    "SKY_TOP_CONNECTED": "上端につながる領域だけを採用します。空だけを検出したい場合の誤検出抑制用です。人物なども対象にする場合はOFFにします",
...
_TIPS_EN: dict[str, str] = {
    ...
    "YOLO_EXPAND": "Adjust the primary mask boundary by fixed pixels. Default is 2px. Drag horizontally to adjust. Safe range: -16 to 32px",
    ...
    "SKY_MIN_SCORE": "For Mask2Former, only accept selected-class pixels whose score is at least this value. 0 disables this filter. Raise it only when false positives appear",
    "SKY_MIN_AREA": "Remove small sky candidates by image-area ratio. Default is 0.05%. This suppresses tiny false positives",
    "SKY_TOP_CONNECTED": "Keep only regions connected to the top edge. Use this to reduce false positives when detecting sky only; leave it off when people or other targets are also selected",

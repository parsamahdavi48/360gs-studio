...
_JA: dict[str, str] = {
    ...
    "CUSTOM_MASK_FILE_FILTER": "画像ファイル (*.png *.jpg *.jpeg *.tif *.tiff);;すべてのファイル (*.*)",
    ...
    "MASK_REPROCESS_CURRENT_BUTTON": "表示中を再処理",
    "MASK_REPROCESS_SELECTED_BUTTON": "選択中を再処理 ({count})",
    "MASK_REPROCESS_SELECTED_FALLBACK_BUTTON": "選択中を再処理",
    "MASK_REPROCESS_CURRENT_RUNNING": "再処理中...",
    "MASK_REPROCESS_CURRENT_DONE": "再処理しました: {name}",
    "MASK_REPROCESS_SELECTED_PROGRESS": "再処理中: {done}/{total} {name}",
...
_EN: dict[str, str] = {
    ...
    "CUSTOM_MASK_FILE_FILTER": "Image files (*.png *.jpg *.jpeg *.tif *.tiff);;All files (*.*)",
    ...
    "MASK_REPROCESS_CURRENT_BUTTON": "Reprocess Current",
    "MASK_REPROCESS_SELECTED_BUTTON": "Reprocess Selected ({count})",
    "MASK_REPROCESS_SELECTED_FALLBACK_BUTTON": "Reprocess Selected",
    "MASK_REPROCESS_CURRENT_RUNNING": "Reprocessing...",
    "MASK_REPROCESS_CURRENT_DONE": "Reprocessed: {name}",
    "MASK_REPROCESS_SELECTED_PROGRESS": "Reprocessing: {done}/{total} {name}",
...
_TIPS_JA: dict[str, str] = {
    ...
    "MASK_TASK_CUSTOM": "ユーザーが用意した静的マスクを最後にAND合成します。白=採用、黒=除外です。サイズが一致する画像だけに適用し、不一致はスキップします",
    "CUSTOM_MASK_FILE": "全フレームに適用するカスタムマスク画像。サイズが元画像と一致する画像だけに適用され、不一致は自動リサイズせずスキップします。色付き画像はグレースケールとして読み込みます",
    ...
    "AXIS_TRANSFORM": "出力先に合わせてカメラ座標軸を変換します。プリセット値から変更すると出力プロファイルはカスタムになります",
    ...
    "PITCH_ROWS": "垂直方向のPitch行数 (1-5)。左端のPitch角はドラッグで調整できます。各行は -90〜90 度で、重複時は自動でずらします",
    ...
    "MASK_PREVIEW_MODE_SINGLE": "選択中の1枚を大きく表示します。細部確認や表示中の再処理に使います",
    ...
    "MASK_REPROCESS_CURRENT_BUTTON": "1枚プレビューでは現在表示中の1枚、サムネイル一覧では選択中の画像を現在の設定で masks/ に再生成します",
...
_TIPS_EN: dict[str, str] = {
    ...
    "MASK_TASK_CUSTOM": "AND-merge a user-provided static mask as the final step. White means keep and black means exclude. It applies only to images with matching dimensions; mismatches are skipped",
    "CUSTOM_MASK_FILE": "Custom mask image applied to every frame with matching dimensions. Mismatches are skipped without auto-resizing. Color images are read as grayscale",
    ...
    "AXIS_TRANSFORM": "Transform camera axes for the target app. Changing the preset value switches the output profile to Custom",
    ...
    "PITCH_ROWS": "Number of vertical pitch rows (1-5). Drag the left pitch angle fields to adjust. Each row is clamped to -90 to 90 degrees; duplicates are nudged automatically",
    ...
    "MASK_PREVIEW_MODE_SINGLE": "Show the selected image large for detail checks and current-image reprocessing",
    ...
    "MASK_REPROCESS_CURRENT_BUTTON": "In single preview, rebuild the current image. In thumbnails, rebuild selected images with the current settings and save to masks/",

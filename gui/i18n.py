...
_JA: dict[str, str] = {
    ...
    "MS_PLY_APPROVE_CANDIDATE": "候補PLYです。使う場合は承認してください。",
    "MS_PLY_APPROVED": "承認済み",
    "MS_PLY_APPROVAL_HINT": "候補PLYです。チェックボタンで承認してください。",
    ...
    "STEP4_PIPELINE_INTENT_METASHAPE_INPUT": "Cubeに必要なMetashape入力です。SfM実行コマンドはないためON固定です。",
    ...
    "STEP4_PIPELINE_METASHAPE_CUBE_ENABLES_INPUT": "CubeはXML/PLYを使うため、ONにするとMetashape入力確認もONになります。",
    "STEP4_PIPELINE_METASHAPE_CUBE_DISABLES_INPUT": "XML/PLYを使うCubeをOFFにすると、Metashape入力確認もOFFになります。",
    "STEP4_PIPELINE_NOTICE_METASHAPE_ENABLED_INPUT": "CubeはXML/PLYを使うため\nMetashape入力確認もONにしました",
    "STEP4_PIPELINE_NOTICE_METASHAPE_DISABLED_INPUT": "XML/PLYを使うCubeをOFFにしたため\nMetashape入力確認もOFFにしました",
    "STEP4_PIPELINE_COLMAP_SFM_ENABLES_CUBE": "COLMAP SfMはCubeで作る視点画像を使うため、ONにするとCubeもONになります。",
    "STEP4_PIPELINE_COLMAP_CUBE_DISABLES_SFM": "CubeをOFFにすると視点画像を作らないため、COLMAP SfMもOFFになります。",
    "STEP4_PIPELINE_NOTICE_COLMAP_ENABLED_CUBE": "COLMAP SfMはCube画像を使うため\nCubeもONにしました",
    "STEP4_PIPELINE_NOTICE_COLMAP_DISABLED_SFM": "Cube画像の書き出しをOFFにしたため\nCOLMAP SfMもOFFにしました",
    ...
    "STEP4_PIPELINE_DETAIL_METASHAPE_NEEDS_PLY_APPROVAL": "候補PLYを使う場合はチェックボタンで承認してください。",
...
_EN: dict[str, str] = {
    ...
    "MS_PLY_APPROVE_CANDIDATE": "Candidate PLY. Approve it if this is the file to use.",
    "MS_PLY_APPROVED": "Approved",
    "MS_PLY_APPROVAL_HINT": "Candidate PLY. Approve it with the check button.",
    ...
    "STEP4_PIPELINE_INTENT_METASHAPE_INPUT": "Metashape input required by Cube. No SfM command is run, so this stays on.",
    ...
    "STEP4_PIPELINE_METASHAPE_CUBE_ENABLES_INPUT": "Cube uses XML/PLY, so enabling it also enables the Metashape input check.",
    "STEP4_PIPELINE_METASHAPE_CUBE_DISABLES_INPUT": "Turning off Cube that uses XML/PLY also turns off the Metashape input check.",
    "STEP4_PIPELINE_NOTICE_METASHAPE_ENABLED_INPUT": "Cube uses XML/PLY\nMetashape input check is on",
    "STEP4_PIPELINE_NOTICE_METASHAPE_DISABLED_INPUT": "Cube using XML/PLY is off\nMetashape input check is off",
    "STEP4_PIPELINE_COLMAP_SFM_ENABLES_CUBE": "COLMAP SfM uses the view images created by Cube, so enabling SfM also enables Cube.",
    "STEP4_PIPELINE_COLMAP_CUBE_DISABLES_SFM": "Turning off Cube skips the view images, so COLMAP SfM is turned off too.",
    "STEP4_PIPELINE_NOTICE_COLMAP_ENABLED_CUBE": "COLMAP SfM uses Cube images\nCube is on too",
    "STEP4_PIPELINE_NOTICE_COLMAP_DISABLED_SFM": "Cube image export is off\nCOLMAP SfM is off too",
    "STEP4_PIPELINE_ROW_TOOLTIP": "{stage}: {status}\n{detail}\nUse the left circle to include or skip this stage.",
    ...
    "STEP4_PIPELINE_DETAIL_METASHAPE_NEEDS_PLY_APPROVAL": "Approve the candidate PLY with the check button before using it.",
...
_TIPS_JA: dict[str, str] = {
    ...
    "MS_PLY": "点群PLY。シーン直下のPLYを候補にします。候補を使う場合はチェックボタンで承認します。pointcloud.ply は除外します",
    "SFM_INPUT_SECTION": "Cubeや後段トレーニングが参照するSfM結果を選びます",
    "COLMAP_SPARSE_MODEL": "COLMAP routeで使うsparseモデルフォルダ。未選択なら output/colmap_rig/sparse/ から自動検出します。COLMAP routeは3DGUT出力には使いません",
...
_TIPS_EN: dict[str, str] = {
    ...
    "MS_PLY": "Point-cloud PLY. PLY files at the scene root are shown as candidates. Approve a candidate with the check button before use. pointcloud.ply is excluded.",
    "SFM_INPUT_SECTION": "Choose the SfM result used by Cube and later training stages",
    "COLMAP_SPARSE_MODEL": "Sparse model folder for the COLMAP route. If empty, output/colmap_rig/sparse/ is auto-detected. The COLMAP route is projected-only and does not produce 3DGUT output.",

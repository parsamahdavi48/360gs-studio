...
_TIPS_JA: dict[str, str] = {
    ...
    "OUTPUT_DIR_SPHERESFM_PROJECT": "SphereSfMの作業用ファイルはシーン内にまとめます。完成データは3DGUTならシーン直下、投影視点なら output/ 直下に作成します",
    ...
    "METHOD_SPHERESFM": "抽出済みエクイレクタングラー画像をSphereSfMでSfMし、その結果から3DGUT用データまたは投影視点データを書き出します",
    ...
    "SPHERESFM_OPEN_GUI": "SphereSfMのSfM結果をCOLMAP GUIで開き、カメラ位置と点群を確認します。GUI機能付きのSphereSfM版 colmap.exe が必要です",
    "SPHERESFM_OUTPUT_SHAPE": "SphereSfMのSfM結果を、LichtFeld 3DGUT用データにするか、投影視点データにするかを選びます",
...
_TIPS_EN: dict[str, str] = {
    ...
    "OUTPUT_DIR_SPHERESFM_PROJECT": "Keeps SphereSfM working files inside the scene. Final data is written at the scene root for 3DGUT, or under output/ for projected views.",
    ...
    "METHOD_SPHERESFM": "Run SfM on extracted equirectangular images with SphereSfM, then export either 3DGUT data or projected-view data from the result.",
    ...
    "SPHERESFM_OPEN_GUI": "Open the SphereSfM SfM result in COLMAP GUI to inspect camera positions and sparse points. Requires a SphereSfM colmap executable with GUI support.",
    "SPHERESFM_OUTPUT_SHAPE": "Choose whether the SphereSfM SfM result becomes LichtFeld 3DGUT data or projected-view data.",

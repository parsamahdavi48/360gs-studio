...
_TIPS_JA: dict[str, str] = {
    ...
    "MASK_REPROCESS_CURRENT_BUTTON": "現在表示中の1枚だけ、現在の設定でマスクを作り直して masks/ に保存します。全体をやり直さずに漏れを直したいときに使います",
...
_TIPS_EN: dict[str, str] = {
    ...
    "MASK_REPROCESS_CURRENT_BUTTON": "Rebuild the mask for only the currently displayed image with the current settings and save it to masks/. Use it to fix misses without rerunning the whole set",

...
_TIPS_JA: dict[str, str] = {
    ...
    "MASK_TASK_STITCH_DISABLED_NORMAL": "スティッチ境界は360エクイレクタングラー画像専用です。通常画像では使いません",
    "MASK_TASK_OVEREXPOSURE": "白飛びした画素を検出してマスクに追加。室内照明では消しすぎる場合があるため必要な時だけON",
    "MASK_IMAGE_TYPE": "360画像か通常画像かを選びます。通常画像ではスティッチ境界と360底面再検出を使いません",
    "MASK_IMAGE_TYPE_EQUIRECT": "エクイレクタングラー360画像。スティッチ境界マスクと下方向の再検出を使えます",
    "MASK_IMAGE_TYPE_NORMAL": "通常の動画フレームやデジタル一眼の連番画像。スティッチ境界と360底面再検出を無効化します",
    ...
    "YOLO_BOTTOM_ENHANCE": "360画像の下方向だけ検出を強化します。強は低conf+4方向回転TTA+近傍2フレーム補完、最大はさらに底面だけYOLO Xと近傍4フレーム補完を使います",
...
_TIPS_EN: dict[str, str] = {
    ...
    "YOLO_BOTTOM_ENHANCE": "Strengthens only the downward 360 bottom detection. Strong uses lower confidence, 4-rotation TTA, and 2-neighbor temporal fill; Max also uses YOLO X for bottom detection and a 4-neighbor temporal fill",

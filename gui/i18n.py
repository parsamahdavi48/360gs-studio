...
_TIPS_JA: dict[str, str] = {
    ...
    "SAM31_APPLY_MODE": "SAM3.1専用。再生成は現在の検出結果でマスクを書き直します。加算は既存マスクに検出領域を黒で追加し、減算は検出領域を白に戻します",
...
_TIPS_EN: dict[str, str] = {
    ...
    "SAM31_APPLY_MODE": "SAM3.1 only. Replace rewrites the mask from the current detections. Add blackens detected regions in the existing mask. Subtract turns detected regions white in the existing mask",

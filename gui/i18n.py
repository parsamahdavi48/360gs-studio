...
_JA: dict[str, str] = {
    ...
    "PHASE_TRAINING_LICHTFELD": "LichtFeld学習",
    "PHASE_TRAINING_POSTSHOT": "Postshot学習",
    "PHASE_TRAINING_CUSTOM": "カスタム学習",
    ...
    "TRAINING_EXEC_NOT_FOUND": "学習ソフトの実行ファイルが見つかりません。インストール先の実行ファイルを指定してください: {path}",
    "TRAINING_REQUIRES_DATASET_OUTPUT": "SfMのみでは学習用データが作成されません。学習を続けて実行する場合は「SfM + 変換」または「既存SfMから変換のみ」を選んでください。",
    ...
    "RUN_TRAINING_AFTER_EXPORT": "書き出し後に学習を開始",
    "TRAINING_EXECUTABLE": "実行ファイル",
    "TRAINING_DATASET": "入力データ",
    "TRAINING_OUTPUT": "学習出力先",
...
_TIPS_JA: dict[str, str] = {
    ...
    "TRAINING_BACKEND_LICHTFELD": "LichtFeld StudioのCLIを起動し、書き出したデータセットで3DGS学習まで続けます",
    "TRAINING_BACKEND_POSTSHOT": "Postshot CLIを起動し、画像と推定済みCOLMAPポーズを渡して学習プロジェクトを作成します",
    "TRAINING_BACKEND_CUSTOM": "任意のCLIを指定します。データセットや出力先はテンプレートのプレースホルダーで渡せます",
    "RUN_TRAINING_AFTER_EXPORT": "ONにするとStep 4の書き出しまたはSfM変換が完了したあと、選択中の学習ソフトを続けて実行します",
    "TRAINING_EXECUTABLE": "学習ソフトの実行ファイル。空欄なら既定名または既知のインストール先を使います",
    "TRAINING_DATASET": "学習ソフトへ渡すデータセットフォルダ。通常は現在のルートと出力形状から自動設定されます",
    "TRAINING_OUTPUT": "学習結果やプロジェクトファイルを書き込むフォルダ。ルートと学習ソフトごとに分けて保存します",
    "TRAINING_HEADLESS": "LichtFeld Studioを画面表示なしで起動します。対応ビルドで自動学習したい場合に使います",
    "LFS_STRATEGY": "LichtFeldの学習方式。まずは既定のMRNFから始め、必要に応じてMCMCやIGS+へ切り替えます",
    "LFS_ITERATIONS": "学習反復回数。大きいほど時間がかかります。最終品質を詰めるときに増やします",
    ...
    "LFS_STEPS_SCALER": "LichtFeld側のスケジュール全体に掛ける倍率。データ量に合わせて学習ステップを伸縮します",
    ...
    "POSTSHOT_KSTEPS": "Postshot CLIへ渡す学習ステップ数です。単位はkStepsです",
...
_TIPS_EN: dict[str, str] = {
    ...
    "LFS_STEPS_SCALER": "Multiplier for LichtFeld's training schedule. Use it to stretch or shrink step counts for the dataset size.",

...
_TIPS_JA: dict[str, str] = {
    ...
    "OUTPUT_DIR_LICHTFELD_DIRECT": "シーンフォルダをLichtFeld 3DGUT用データセットとして使います。images/ と masks/ は元のまま参照し、transforms.json と pointcloud.ply をシーン直下に作成します",
    "OUTPUT_DIR_COLMAP_PROJECT": "シーンフォルダ内の output/colmap_rig/ が完成COLMAPプロジェクトです。COLMAP実行後はこのフォルダを3DGSアプリへ渡します",
    "OUTPUT_DIR_SPHERESFM_PROJECT": "SphereSfMのdatabase、マスク変換結果、sparseモデルは output/spheresfm/ にまとめ、変換成果物は3DGUTならシーン直下、投影視点なら output/ 直下に作成します",
    ...
    "METHOD_SPHERESFM": "抽出済みエクイレクタングラー画像をSphereSfM版COLMAPでSfMし、その結果を変換設定の出力形状に従って3DGUTまたは投影視点データまで書き出します",
    ...
    "SPHERESFM_EXECUTABLE": "SphereSfMをビルドまたは配布版から入手した colmap.exe。通常のCOLMAPではなく、sphere_cubic_reprojecer を持つ実行ファイルが必要です",
    "SPHERESFM_USE_MASKS": "ONにするとStep 3の masks/ をSphereSfM/COLMAPの image.jpg.png 命名へ変換し、feature_extractor の mask_path に渡します",
    "SPHERESFM_RUN_SCOPE": "SfMから変換まで続けるか、SfMだけを作り直すか、既存のSphereSfM sparseモデルから変換だけをやり直すかを選びます",
    ...
    "OUTPUT_SHAPE": "Metashape XML/PLYから投影視点画像を作るか、LichtFeld 3DGUT向けに元のエクイレクタングラー画像をそのまま使うかを選びます",
...
_TIPS_EN: dict[str, str] = {
    ...
    "OUTPUT_DIR_LICHTFELD_DIRECT": "Uses the scene folder itself as the LichtFeld 3DGUT dataset. images/ and masks/ are referenced in place, while transforms.json and pointcloud.ply are written at the scene root.",
    ...
    "OUTPUT_DIR_SPHERESFM_PROJECT": "Keeps the SphereSfM database, converted masks, and sparse model under output/spheresfm/, while final conversion output goes to the scene root for 3DGUT or output/ for projected views.",
    ...
    "METHOD_SPHERESFM": "Run SfM on extracted equirectangular images with SphereSfM's COLMAP build, then continue through the conversion settings to export either 3DGUT or projected-view data.",
    ...
    "SPHERESFM_EXECUTABLE": "Path to SphereSfM's colmap executable. This must be the SphereSfM build that includes sphere_cubic_reprojecer, not a standard COLMAP binary.",
    "SPHERESFM_USE_MASKS": "Converts Step 3 masks/ to COLMAP's image.jpg.png naming and passes them to feature_extractor as mask_path.",
    "SPHERESFM_RUN_SCOPE": "Choose whether to run SfM and conversion together, rebuild only the SfM result, or rerun conversion from an existing SphereSfM sparse model.",
    ...
    "OUTPUT_SHAPE": "Choose whether Metashape XML/PLY is converted into projection views or kept as source equirectangular images for LichtFeld 3DGUT.",

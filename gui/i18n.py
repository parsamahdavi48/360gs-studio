...
_TIPS_JA: dict[str, str] = {
    ...
    "MASK_TASK_CUSTOM": "ユーザーが用意した静的マスクを最後にAND合成します。白=採用、黒=除外です。未選択のままONにするとファイル選択を開きます",
    "CUSTOM_MASK_FILE": "全フレームに適用するカスタムマスク画像。画像サイズは元画像と一致している必要があります。色付き画像はグレースケールとして読み込みます",
...
_TIPS_EN: dict[str, str] = {
    ...
    "MASK_TASK_CUSTOM": "AND-merge a user-provided static mask as the final step. White means keep and black means exclude. Turning this on without a file opens the file picker",
    "CUSTOM_MASK_FILE": "Custom mask image applied to every frame. Its size must match the source images. Color images are read as grayscale",

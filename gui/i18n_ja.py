...
STRINGS: dict[str, str] = {
    ...
    "SFM_MENU_DESC": "学習用データセットの元になるSfM結果を選びます。Metashape / RealityScan / COLMAP / SphereSfMなどの既存結果を使うか、このアプリからCOLMAP / SphereSfMを実行して作成します。",
    ...
    "SFM_ROUTE_REALITYSCAN_BODY": "MetashapeのSfM結果から、RealityScanで再アラインするためのCubemap画像とXMPを作成します。",
    ...
    "SFM_ROUTE_SPHERESFM_BODY": "同一解像度のエクイレクタングラー形式360°画像をSphereSfMで処理し、カメラポーズと疎点群を作成します。",
    ...
    "SFM_SPHERESFM_DETAIL_DESC": "同一解像度の360°画像をSphereSfM版COLMAPで処理し、カメラポーズと疎点群を含むsparseモデルを作成します。作成済みsparseからのCubemap/GUT変換は次のデータセット工程で行います。",
    "SFM_REALITYSCAN_DETAIL_TITLE": "Metashape → RealityScan用データ作成",
    "SFM_REALITYSCAN_DETAIL_DESC": "MetashapeのSfM結果と元画像・マスクから、RealityScanに渡すCubemap画像とXMPを作成します。ここで作った output/realityscan/ をRealityScanへ読み込み、Align後にCSV/PLYを書き出します。",
    # Step 5
...
TIPS: dict[str, str] = {
    ...
    "RUN_MASKS": "処理範囲で選ばれた画像に、主マスク生成と現在ONの追加マスク処理を実行します。既存マスクを上書きしたくない場合は「マスク未作成の画像のみ」を選びます",

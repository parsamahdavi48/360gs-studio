...
STRINGS: dict[str, str] = {
    ...
    "METASHAPE_NOTICE": (
        ...
        "SphereSfMルートではデータセット工程でSfMから変換まで実行し、3DGUT用データまたはキューブマップデータを作ります。\n"
    ...
    "RUN_ALL": "全マスク実行",
    # Step 4
    "SFM_MENU_TITLE": "SfM / カメラアライメント",
    "SFM_MENU_DESC": "カメラ位置を作る方法を選びます。COLMAP / SphereSfM はこの工程で実行し、外部SfM結果がある場合だけ次のデータセット工程へ進みます。",
    ...
    "SFM_ROUTE_EXTERNAL_TITLE": "外部SfM結果を使う",
    "SFM_ROUTE_EXTERNAL_BODY": "Metashape / COLMAP / SphereSfM / RealityScan などで作成済みの結果を使います。Step 4で実行する処理はないので、データセット作成へ進みます。",
    "SFM_ROUTE_EXTERNAL_FOOTER": "次: データセット工程で入力元を選択",
    "SFM_ROUTE_COLMAP_TITLE": "COLMAPでSfM",
    "SFM_ROUTE_COLMAP_BODY": "Cubemap画像を準備し、COLMAPまたはGLOMAPでカメラ位置を推定します。",
    "SFM_ROUTE_COLMAP_FOOTER": "出力: output/colmap_rig/",
    "SFM_ROUTE_SPHERESFM_TITLE": "SphereSfM",
    "SFM_ROUTE_SPHERESFM_BODY": "360画像からSphereSfM版COLMAPでカメラ位置を推定します。",
    ...
    "SFM_COLMAP_DETAIL_TITLE": "COLMAPでSfM",
    ...
    "SFM_SPHERESFM_DETAIL_DESC": "抽出済みの360画像をSphereSfM版COLMAPで処理して、sparseモデルを作成します。作成済みsparseからのCubemap/3DGUT変換は次のデータセット工程で行います。",
    # Step 5
    "DATASET_MENU_TITLE": "学習用データセット作成",
    "DATASET_MENU_DESC": "学習前に必要な変換や補正を、実際の入力元ごとに選びます。COLMAPで作成した output/colmap_rig/ はこの工程を飛ばして学習へ進めます。",
    "DATASET_SELECT_TOOL": "ツールを選択",
    "DATASET_BACK_TO_MENU": "← データセット一覧",
    "DATASET_TOOL_METASHAPE_TITLE": "Metashape結果 → Cubemap / 3DGUT",
    "DATASET_TOOL_METASHAPE_DESC": "MetashapeのXML/PLYと元画像・マスクから、Postshot / Brush / LichtFeld向けの学習用データセットを作成します。",
    "DATASET_TOOL_METASHAPE_CARD_BODY": "Metashapeで作成済みのカメラ結果を、学習用のCubemapまたは3DGUT形式へ変換します。",
    "DATASET_TOOL_METASHAPE_CARD_FOOTER": "入力: XML / PLY / images / masks",
    "DATASET_TOOL_SPHERESFM_TITLE": "SphereSfM結果 → 学習用データセット",
    "DATASET_TOOL_SPHERESFM_DESC": "Step 4で作成したSphereSfM sparseモデル、または指定したSphereSfM sparseモデルから、Cubemapまたは3DGUT形式の学習用データセットを作成します。",
    "DATASET_TOOL_SPHERESFM_CARD_BODY": "SphereSfMのsparseモデルを、LichtFeld / Postshot / Brush向けの学習用データへ変換します。",
    ...
    "DATASET_TOOL_RS_LFS_DESC": "RealityScanから書き出したCSVとPLYを、LichtFeldがDatasetとして読めるCOLMAP形式へ変換します。元画像とマスクはコピーせず、リンクまたは必要な変換だけを行います。",
    "DATASET_TOOL_RS_LFS_CARD_BODY": "RealityScanのCSV/PLYからLichtFeld用COLMAPデータセットを作成します。",
    ...
    "RS_LFS_MASKS": "masks",
    "RS_LFS_OUTPUT": "出力先",
    "RS_LFS_DISTORTION_MODE": "歪み画像",
    "RS_LFS_PRE_UNDISTORT": "事前UndistortしてPINHOLE化",
    "RS_LFS_UNDISTORT_ALPHA": "Undistort余白",
...
TIPS: dict[str, str] = {
    ...
    "SFM_ROUTE_EXTERNAL": "Metashape、COLMAP、SphereSfM、RealityScanなどで作成済みのカメラ結果を使うルートです。Step 4で実行する処理はないためデータセット工程へ進みます",
    "SFM_ROUTE_COLMAP": "このアプリでCubemap画像を用意してCOLMAP/GLOMAPを実行するルートです。詳細はStep 4内で設定します",
    "SFM_ROUTE_SPHERESFM": "360画像をSphereSfM版COLMAPで処理するルートです。SfM実行はStep 4で行い、変換はデータセット工程で行います",
    "SFM_ROUTE_VIEWER": "作成済みのカメラ位置、点群、transforms.jsonの整合を確認するビューを開きます",
    "DATASET_SELECT_TOOL": "カードから作成または補正したいデータセット処理を選びます",
    "DATASET_TOOL_METASHAPE": "Metashapeで作成済みのXML/PLYを入力にして、Cubemapまたは3DGUTの学習用データセットを作ります",
    "DATASET_TOOL_SPHERESFM": "SphereSfMのsparseモデルを入力にして、Cubemapまたは3DGUTの学習用データセットを作ります。SfM自体の実行はStep 4で行います",
    "DATASET_COLMAP_READY_ACTION": "COLMAP結果はこの工程で実行する処理がありません。左のステップから学習工程へ進みます",
    ...
    "DATASET_TOOL_RS_LFS": "RealityScanからPostshot用に書き出したCSV/PLYを、LichtFeldのDataset読み込み用COLMAP形式へ変換します",
    ...
    "RS_LFS_PLY": "RealityScanからCSVと同じ座標系で書き出したPLYです。LichtFeld用COLMAP作成時に点群へ必要な向き変換を適用します",
    ...
    "RS_LFS_OUTPUT": "LichtFeldでDatasetとして開くフォルダです。中に images/, masks/, sparse/0/ を作成します",
    "RS_LFS_PRE_UNDISTORT": "RealityScan CSV内で歪み係数を持つ通常画像だけを事前にUndistortし、COLMAPにはPINHOLEとして書き出します。Cubemap由来のPINHOLE画像はリンクのままです",
    "RS_LFS_UNDISTORT_ALPHA": "0は黒縁を抑えてクロップ、1は視野を広く残します。通常は0から始めます",
...
TIPS.update(
    {
        ...
        "LFS_GUT": "3DGUT用データで学習します。歪みの少ない360画像を直接扱えますが、通常のCubemap学習よりVRAM負荷が高くなりやすいです",

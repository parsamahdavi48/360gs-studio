...
STRINGS: dict[str, str] = {
    ...
    "SFM_MENU_DESC": "カメラ位置を作る方法を選びます。アプリ内で実行するルートも、外部SfM結果を使うルートも、詳細設定と実行は次のデータセット工程で行います。",
    "SFM_SELECT_ROUTE": "方式を選択",
    "SFM_ROUTE_EXTERNAL_TITLE": "外部SfM結果を使う",
    "SFM_ROUTE_EXTERNAL_BODY": "Metashapeなどで作成済みのカメラ結果からデータセットを作成します。",
    "SFM_ROUTE_EXTERNAL_FOOTER": "入力: XML / PLY",
    ...
    "DATASET_MENU_DESC": "SfM結果、外部ツールの出力、既存データセットに対して、学習前に必要な変換や補正を行います。",
    "DATASET_SELECT_TOOL": "ツールを選択",
    "DATASET_BACK_TO_MENU": "← データセット一覧",
    "DATASET_TOOL_CUBEMAP_TITLE": "SfM結果 → データセット",
    "DATASET_TOOL_CUBEMAP_CARD_BODY": "Metashape / COLMAP / SphereSfM の結果からCubemap、3DGUT、Postshot/Brush/LichtFeld向け出力を作成します。",
    "DATASET_TOOL_CUBEMAP_CARD_FOOTER": "入力: SfM結果 / images / masks",
...
TIPS: dict[str, str] = {
    ...
    "CANCEL": "実行中の処理を中断します",
    "SFM_SELECT_ROUTE": "この画面のカードからSfMルートを選び、次のデータセット工程で設定と実行を行います",
    "SFM_ROUTE_EXTERNAL": "外部SfMソフトで作成済みのカメラ結果を使うルートです。Metashape XML/PLYなどをデータセット工程で指定します",
    "SFM_ROUTE_COLMAP": "このアプリでCubemap画像を用意してCOLMAP/GLOMAPを実行するルートです。SfM実行とデータセット作成はデータセット工程でまとめて設定します",
    "SFM_ROUTE_SPHERESFM": "360画像をSphereSfM版COLMAPで処理するルートです。SfMのみ、またはSfM後のデータセット作成までをデータセット工程で選べます",
    "DATASET_SELECT_TOOL": "カードから作成または補正したいデータセット処理を選びます",
    "DATASET_TOOL_CUBEMAP": "既存のMetashape/COLMAP/SphereSfMワークフローです。SfM結果の指定、Cubemap/3DGUT出力、COLMAP/GLOMAP実行をここで扱います",

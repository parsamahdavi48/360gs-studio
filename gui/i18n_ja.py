...
STRINGS: dict[str, str] = {
    ...
    "METASHAPE_NOTICE": (
        ...
        "Metashapeルートでは masks/ をマスクとして読み込み、SfM後にデータセット工程で3DGS向けに書き出します。\n"
        "SphereSfMルートではデータセット工程でSfMから変換まで実行し、GUT用データまたはキューブマップデータを作ります。\n"
        "COLMAPルートではデータセット工程でキューブマップ画像を書き出し、必要に応じてCOLMAP/GLOMAPを実行します。"
    ),
    "EXPORT_DIR": "エクスポート先",
    # Step 3
    "IMAGES_DIR": "画像フォルダ",
    ...
    "MASK_SCOPE": "処理範囲",
    "MASK_SCOPE_MISSING": "未作成のみ",
    "MASK_SCOPE_STALE": "未作成・古いもの",
    "MASK_SCOPE_ALL": "すべて再生成",
    ...
    "RUN_ALL": "全マスク実行",
    # Step 4
    "SFM_MENU_TITLE": "SfM実行（必要な場合のみ）",
    "SFM_MENU_DESC": (
        "この工程は、カメラ位置をまだ作っていない場合だけ使います。"
        "Metashape / RealityScan / COLMAP などでSfM済みなら、ここでは何も実行せず次のデータセット工程へ進みます。"
        "RealityScanで再アラインするためのMetashape→XMP作成、またはこのアプリから COLMAP / SphereSfM をCLI起動する場合だけ、下のカードを選びます。"
    ),
    "SFM_SELECT_ROUTE": "方式を選択",
    "SFM_BACK_TO_ROUTES": "← SfM方式一覧",
    "SFM_RUN_COLMAP": "COLMAP SfMを実行",
    "SFM_RUN_SPHERESFM": "SphereSfMを実行",
    "SFM_RUN_REALITYSCAN": "RealityScan用データを作成",
    "SFM_OPEN_VIEWER": "SfM結果を確認",
    "SFM_ROUTE_EXTERNAL_TITLE": "既存のSfM結果を使う",
    "SFM_ROUTE_EXTERNAL_BODY": "Metashape / RealityScan / COLMAP / SphereSfM などで作成済みの結果がある場合のルートです。Step 4では何も実行せず、次の工程で入力元を選びます。",
    "SFM_ROUTE_EXTERNAL_FOOTER": "次: データセット工程で入力元を選択",
    "SFM_ROUTE_REALITYSCAN_TITLE": "Metashape → RealityScan用データ作成",
    "SFM_ROUTE_REALITYSCAN_BODY": "Metashapeで作成済みのカメラXMLから、RealityScanで再アラインするためのCubemap画像とXMPを作成します。",
    "SFM_ROUTE_REALITYSCAN_FOOTER": "出力: output/realityscan/",
    "SFM_ROUTE_COLMAP_TITLE": "COLMAPでSfMを実行",
    "SFM_ROUTE_COLMAP_BODY": "エクイレクタングラー形式の360°画像をCubemap視点画像に展開し、COLMAP RigとしてSfMします。カメラ位置と疎点群をこの工程で作成します。",
    "SFM_ROUTE_COLMAP_FOOTER": "出力: output/colmap_rig/",
    "SFM_ROUTE_SPHERESFM_TITLE": "SphereSfMでSfMを実行",
    "SFM_ROUTE_SPHERESFM_BODY": "エクイレクタングラー形式の360°画像をCubemapに展開せず、そのままSphereSfMでSfMします。カメラ位置と疎点群をこの工程で作成します。",
    "SFM_ROUTE_SPHERESFM_FOOTER": "出力: output/spheresfm/",
    "SFM_ROUTE_VIEWER_TITLE": "SfM結果を確認",
    "SFM_ROUTE_VIEWER_BODY": "作成済みのカメラ、点群、データセット出力を読み取り専用ビューで確認します。",
    "SFM_ROUTE_VIEWER_FOOTER": "確認: sparse / transforms / pointcloud",
    "SFM_COLMAP_DETAIL_TITLE": "COLMAPでSfMを実行",
    "SFM_COLMAP_DETAIL_DESC": "COLMAP用のCubemap視点画像を自動で準備し、そのままCOLMAPまたはGLOMAPでカメラ位置を推定します。生成後の学習用データセット作成は次の工程で行います。",
    "SFM_SPHERESFM_DETAIL_TITLE": "SphereSfMでSfMを実行",
    "SFM_SPHERESFM_DETAIL_DESC": "抽出済みの360画像をSphereSfM版COLMAPで処理して、sparseモデルを作成します。作成済みsparseからのCubemap/GUT変換は次のデータセット工程で行います。",
    "SFM_REALITYSCAN_DETAIL_TITLE": "Metashape → RealityScan用データ作成",
    "SFM_REALITYSCAN_DETAIL_DESC": "MetashapeのカメラXMLと元画像・マスクから、RealityScanに渡すCubemap画像とXMPを作成します。ここで作った output/realityscan/ をRealityScanへ読み込み、Align後にCSV/PLYを書き出します。",
    # Step 5
    ...
    "DATASET_TOOL_METASHAPE_DESC": "MetashapeのカメラXML/点群PLYから、transforms.json と pointcloud.ply を使うNeRF/3DGS系データセットを作成します。出力タブでPINHOLEまたはERP 360°を選びます。",
    "DATASET_TOOL_METASHAPE_CARD_BODY": "Metashapeで作成済みのカメラ結果を、NeRF/3DGS系のJSON/PLYデータセットへ変換します。",
    "DATASET_TOOL_METASHAPE_CARD_FOOTER": "形式: transforms.json / pointcloud.ply",
    "DATASET_TOOL_SPHERESFM_TITLE": "SphereSfM → NeRFデータセット(JSON/PLY)",
    "DATASET_TOOL_SPHERESFM_DESC": "Step 4で作成したSphereSfM sparseモデル、または指定したSphereSfM sparseモデルから、transforms.json と pointcloud.ply を使うNeRF/3DGS系データセットを作成します。",
    "DATASET_TOOL_SPHERESFM_CARD_BODY": "SphereSfMのsparseモデルを、NeRF/3DGS系のJSON/PLYデータセットへ変換します。",
    "DATASET_TOOL_SPHERESFM_CARD_FOOTER": "入力: output/spheresfm/sparse/",
    ...
    "COLMAP_NORMAL_CAMERA": "通常画像カメラ",
    "COLMAP_NORMAL_CAMERA_AUTO": "自動推定",
    "COLMAP_NORMAL_CAMERA_SCOPE_DEFAULT": "シーン全体",
    "COLMAP_NORMAL_CAMERA_SCOPE_GROUP_FORMAT": "{source} / {size} / {count}枚",
    "COLMAP_NORMAL_CAMERA_PARAMS_PLACEHOLDER": "fx,fy,cx,cy など",
    "COLMAP_NORMAL_CAMERA_APPLY": "保存",
    "COLMAP_NORMAL_CAMERA_PARAMS_INVALID": "通常画像カメラのparamsを確認してください: {value}\n{detail}",
    "COLMAP_NORMAL_CAMERA_MODEL_REQUIRED": "通常画像カメラのparamsを指定する場合は、カメラモデルも選んでください。",
    "PHASE_COLMAP_RIG_EXPORT": "COLMAP用視点画像を書き出し",
    "PHASE_COLMAP_MIXED_PREPARE": "COLMAP混在プロジェクト準備",
    "PHASE_METASHAPE_IMPORT": "Metashapeインポート",
    "PHASE_METASHAPE_NERF": "Metashape混在データセット作成",
    ...
    "TRAINING_TOOL_LICHTFELD_CARD_BODY": "対応データセットを読み込み、ヘッドレス実行で学習済みPLYを作成できます。画面操作なしで長時間学習を走らせたい場合に使います。",
    "TRAINING_TOOL_LICHTFELD_CARD_FOOTER": "出力: output/",
    "TRAINING_TOOL_LICHTFELD_DESC": "LichtFeld Studioへデータセットを渡してトレーニングを開始します。ヘッドレス実行では画面を表示せずに学習済みPLYを作成できます。",
    "TRAINING_TOOL_POSTSHOT_CARD_BODY": "Postshot CLIにデータセットを渡し、Postshotプロジェクトの作成と学習を実行します。Postshot形式で結果を作りたい場合に使います。",
    "TRAINING_TOOL_POSTSHOT_CARD_FOOTER": "出力: output/*.psht",
    ...
    "STEP4_PIPELINE_DETAIL_SPHERESFM_OFF": "今回はSphereSfMを実行しません。必要なら丸ボタンをクリックしてください。",
...
TIPS: dict[str, str] = {
    ...
    "CANCEL": "実行中の処理を中断します",
    "SFM_SELECT_ROUTE": "この画面のカードからSfMルートを選びます。COLMAPとSphereSfMはこの工程内で設定して実行します",
    "SFM_BACK_TO_ROUTES": "SfM方式を選ぶカード一覧に戻ります",
    "SFM_RUN_COLMAP": "Cubemap視点画像の準備とCOLMAP/GLOMAPのSfMを実行します",
    "SFM_RUN_SPHERESFM": "SphereSfM版COLMAPで360画像からsparseモデルを作成します",
    ...
    "SFM_ROUTE_EXTERNAL": "Metashape、RealityScan、COLMAP、SphereSfMなどで作成済みのカメラ結果を使うルートです。Step 4で実行する処理はありません",
    "SFM_ROUTE_REALITYSCAN": "元画像とマスクをCubemap化し、Metashapeのカメラ姿勢をRealityScan用XMPとして書き出します。点群PLYは必要な場合だけ入力に使います",
    "SFM_ROUTE_COLMAP": "エクイレクタングラー形式の360°画像をCubemap視点画像に展開し、各フレームの複数視点をCOLMAP Rigとして束ねてSfMします。カメラ位置と疎点群をこの工程で作成します",
    "SFM_ROUTE_SPHERESFM": "出力はCOLMAP sparse形式をベースにしたSphereSfMモデルです。画像はCubemap化せず、360°画像のまま参照します",
    "SFM_ROUTE_VIEWER": "作成済みのカメラ位置、点群、transforms.jsonの整合を確認するビューを開きます",
    "DATASET_SELECT_TOOL": "カードから作成または補正したいデータセット処理を選びます",
    "DATASET_TOOL_METASHAPE": "Metashapeで作成済みのXML/PLYを入力にして、transforms.json と pointcloud.ply を使うNeRF/3DGS系データセットを作ります",
    "DATASET_TOOL_SPHERESFM": "SphereSfMのsparseモデルを入力にして、transforms.json と pointcloud.ply を使うNeRF/3DGS系データセットを作ります。SfM自体の実行はStep 4で行います",
    "DATASET_RUN_METASHAPE": "Metashape XML/PLYと現在の出力設定から学習用データセットを作成します",
    "DATASET_RUN_SPHERESFM": "SphereSfM sparseモデルと現在の出力設定から学習用データセットを作成します",
    ...
    "RS_LFS_IMAGES": "CSV内の画像名に対応する元画像フォルダです。通常は output/realityscan/images/ またはシーンの images/ を使います",
    "RS_LFS_MASKS": "対応するマスクフォルダです。存在する場合だけ指定します。白=使用、黒=除外として扱います",
    "RS_LFS_OUTPUT": "LichtFeldでデータセットとして開くフォルダです。中に images/, masks/, sparse/0/ を作成します",
    "RS_LFS_PRE_UNDISTORT": "RealityScanでCubemap画像だけでなく通常カメラ画像も混ぜて再アラインした場合にONにします。レンズ歪み係数がある画像だけを補正してPINHOLE画像に変換し、LichtFeldの標準学習で扱えるようにします。Cubemap由来のPINHOLE画像は変換せず、Windowsのハードリンクで参照します。OFFのまま歪みカメラが残る場合は、LichtFeld側でGUTまたはUndistort指定が必要です",
    ...
    "ADD_INPUT_IMAGE_SEQUENCE": "既に静止画として用意済みの画像フォルダを入力ソース一覧に追加します。画像は実行時にシーンの images/ へコピーされます",
    ...
    "RUN_MASKS": "処理範囲で選ばれた画像に、主マスク生成と現在ONの追加マスク処理を実行します。既存マスクを残したい場合は未作成のみを選びます",
    ...
    "MASK_SCOPE": "マスク生成の対象を選びます。Step 1でソースを追加した後は未作成のみ、設定を変えて作り直す場合は古いものも更新、完全に作り直す場合だけすべて再生成を選びます",
    "MASK_SCOPE_MISSING": "masks/に対応マスクがない画像だけ生成します。既存マスクや手直し済みマスクは触りません",
    "MASK_SCOPE_STALE": "未作成に加えて、前回生成時から元画像またはマスク設定が変わった画像だけ更新します。手編集された可能性がある既存マスクは保護します",
    "MASK_SCOPE_ALL": "現在の設定で全画像のマスクを作り直します。既存マスクや手編集済みマスクも上書きされます",
    ...
    "EXPORT_METHOD": "データセット工程で行うルートを選びます。Metashape結果の変換、COLMAP用視点画像の書き出し、SphereSfMによるSfMと変換を切り替えます",
    ...
    "SPHERESFM_OUTPUT_SHAPE": "PINHOLEは歪みなし視点画像を書き出します。ERP 360°はLichtFeldのGUT向けに360°エクイレクタングラー画像を使います",
    "SPHERESFM_TARGET_PROFILE": "出力先アプリに合わせたプリセットです。ERP 360°ではLichtFeld向けに固定されます",
    ...
    "COLMAP_NORMAL_CAMERA": "通常画像を360°画像と混ぜてCOLMAP SfMする場合の初期カメラ設定です。自動推定ではCOLMAPに任せます。実測または校正済みの内部パラメータがある場合だけモデルとparamsを指定してください。Cubemap Rig画像には影響しません",
    "TARGET_PROFILE": "出力先アプリに合わせて、画像・カメラ姿勢・追加メタデータの書き出し内容を切り替えます",
    "TARGET_PROFILE_REALITYSCAN": "MetashapeのSfM結果からRealityScan用のCubemap画像と\nXMPカメラ姿勢を作成します。RealityScanで読み込んでAlignし、\nRealityScan側で疎点群を再生成する用途です。",
    ...
    "OUTPUT_SHAPE": "PINHOLEは歪みなし視点画像を書き出します。ERP 360°はLichtFeldのGUT向けに360°エクイレクタングラー画像を使います",

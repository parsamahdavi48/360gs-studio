...
class CubemapStep(BaseStepWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.output_browse.setToolTip("キューブマップ変換後の画像とtransforms.jsonの出力先フォルダ")
        form.addRow(i18n.OUTPUT_DIR, self.output_browse)

        profile_row = QHBoxLayout()
        profile_row.setSpacing(8)
        self.profile_combo = QComboBox()
        self.profile_combo.setToolTip("出力先の3DGSソフトウェアに合わせた座標変換とPLY設定のプリセット")
        ...
        self.scale_combo.setToolTip("出力キューブマップ面のサイズ。Half=入力高さの半分、Full=等倍")
        ...
        self.json_name_edit.setToolTip("出力するカメラパラメータJSONのファイル名")
        ...
        self.mask_browse.setToolTip("入力マスク画像のフォルダ。キューブマップ変換時にマスクも一緒に変換")
        ...
        self.mask_from_alpha_cb.setToolTip("RGBA画像のアルファチャンネルからマスクを自動生成")
        opt_row1.addWidget(self.mask_from_alpha_cb)
        self.no_image_cb = QCheckBox(i18n.NO_IMAGE)
        self.no_image_cb.setToolTip("transforms.jsonのみ生成し、画像変換をスキップ (テスト用)")
        opt_row1.addWidget(self.no_image_cb)
        self.no_transform_cb = QCheckBox(i18n.NO_TRANSFORM)
        self.no_transform_cb.setToolTip("座標軸変換を無効化。LichtFeld Studioで必要")
        ...
        self.duplicate_cb.setToolTip("マージされたチャンク間で同名画像を許可")
        opt_row2.addWidget(self.duplicate_cb)
        self.invert_masks_cb = QCheckBox(i18n.INVERT_MASKS)
        self.invert_masks_cb.setToolTip("出力マスクの白黒を反転 (ソフトウェアの要求に合わせて)")
        ...
        self.preprocess_cb.setToolTip("Metashape XMLからtransforms.jsonを生成してからキューブマップ変換を行う")
        ...
        self.ms_scale_edit.setToolTip("座標系のスケール係数。通常は1.0のまま")
        pp_form.addRow(i18n.SCALE_FACTOR, self.ms_scale_edit)

        self.ms_no_fix_rot_cb = QCheckBox(i18n.NO_FIX_ROTATION)
        self.ms_no_fix_rot_cb.setToolTip("前処理の回転補正を無効化。通常はOFFのまま")

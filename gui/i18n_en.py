...
STRINGS: dict[str, str] = {
    ...
    "METASHAPE_NOTICE": (
        ...
        "For the Metashape route, import masks/ as masks, run SfM, then use the Dataset step for 3DGS export.\n"
        "For the SphereSfM route, the Dataset step runs SfM and conversion, then writes either GUT data or cubemap data.\n"
        "For the COLMAP route, use the Dataset step to export cubemap images and optionally run COLMAP/GLOMAP."
    ),
    ...
    "MASK_SCOPE": "Scope",
    "MASK_SCOPE_MISSING": "Missing only",
    "MASK_SCOPE_STALE": "Missing + stale",
    "MASK_SCOPE_ALL": "Regenerate all",
    ...
    # Step 4
    "SFM_MENU_TITLE": "Run SfM Only If Needed",
    "SFM_MENU_DESC": (
        "Use this step only when camera poses have not been created yet. "
        "If Metashape, RealityScan, COLMAP, or another tool has already produced SfM results, do not run anything here; continue to the dataset step. "
        "Choose a card below only when preparing Metashape results for RealityScan realignment, or when this app should launch COLMAP / SphereSfM from the CLI."
    ),
    "SFM_SELECT_ROUTE": "Choose Route",
    "SFM_BACK_TO_ROUTES": "← SfM Routes",
    "SFM_RUN_COLMAP": "Run COLMAP SfM",
    "SFM_RUN_SPHERESFM": "Run SphereSfM",
    ...
    "SFM_ROUTE_EXTERNAL_BODY": "Use this route when Metashape, RealityScan, COLMAP, SphereSfM, or another tool has already produced SfM results. Step 4 runs nothing; choose the input source in the next step.",
    "SFM_ROUTE_EXTERNAL_FOOTER": "Next: choose the input source in Dataset",
    "SFM_ROUTE_REALITYSCAN_TITLE": "Metashape → RealityScan Data",
    "SFM_ROUTE_REALITYSCAN_BODY": "Create Cubemap images and XMP files for RealityScan realignment from an existing Metashape camera XML.",
    "SFM_ROUTE_REALITYSCAN_FOOTER": "Output: output/realityscan/",
    "SFM_ROUTE_COLMAP_TITLE": "Run COLMAP SfM",
    "SFM_ROUTE_COLMAP_BODY": "Expand equirectangular 360° images into Cubemap viewpoint images, then run SfM as a COLMAP Rig. This step creates camera poses and a sparse point cloud.",
    "SFM_ROUTE_COLMAP_FOOTER": "Output: output/colmap_rig/",
    "SFM_ROUTE_SPHERESFM_TITLE": "Run SphereSfM",
    "SFM_ROUTE_SPHERESFM_BODY": "Run SphereSfM directly on equirectangular 360° images without expanding them to Cubemap views. This step creates camera poses and a sparse point cloud.",
    "SFM_ROUTE_SPHERESFM_FOOTER": "Output: output/spheresfm/",
    "SFM_ROUTE_VIEWER_TITLE": "Inspect SfM Result",
    "SFM_ROUTE_VIEWER_BODY": "Open a read-only viewer for existing cameras, points, and dataset outputs.",
    "SFM_ROUTE_VIEWER_FOOTER": "Inspect: sparse / transforms / pointcloud",
    "SFM_COLMAP_DETAIL_TITLE": "Run COLMAP SfM",
    "SFM_COLMAP_DETAIL_DESC": "Automatically prepare COLMAP Cubemap view images, then estimate poses with COLMAP or GLOMAP. Training dataset conversion is handled in the next step.",
    "SFM_SPHERESFM_DETAIL_TITLE": "Run SphereSfM",
    "SFM_SPHERESFM_DETAIL_DESC": "Process extracted 360 images with the SphereSfM COLMAP build and create a sparse model. Cubemap or GUT conversion from that sparse result is handled in the dataset step.",
    "SFM_REALITYSCAN_DETAIL_TITLE": "Metashape → RealityScan Data",
    "SFM_REALITYSCAN_DETAIL_DESC": "Create Cubemap images and XMP files for RealityScan from a Metashape camera XML plus source images and masks. Import output/realityscan/ into RealityScan, then export CSV/PLY after Align.",
    # Step 5
    ...
    "DATASET_TOOL_METASHAPE_DESC": "Create a NeRF/3DGS-style dataset using transforms.json and pointcloud.ply from a Metashape camera XML and point-cloud PLY. Choose PINHOLE or ERP 360° in the Output tab.",
    "DATASET_TOOL_METASHAPE_CARD_BODY": "Convert camera results prepared in Metashape into a NeRF/3DGS-style JSON/PLY dataset.",
    "DATASET_TOOL_METASHAPE_CARD_FOOTER": "Format: transforms.json / pointcloud.ply",
    "DATASET_TOOL_SPHERESFM_TITLE": "SphereSfM → NeRF Dataset (JSON/PLY)",
    "DATASET_TOOL_SPHERESFM_DESC": "Create a NeRF/3DGS-style dataset using transforms.json and pointcloud.ply from the SphereSfM sparse model made in Step 4, or from another selected SphereSfM sparse model.",
    "DATASET_TOOL_SPHERESFM_CARD_BODY": "Convert a SphereSfM sparse model into a NeRF/3DGS-style JSON/PLY dataset.",
    "DATASET_TOOL_SPHERESFM_CARD_FOOTER": "Input: output/spheresfm/sparse/",
    ...
    "COLMAP_NORMAL_CAMERA": "Normal Image Camera",
    "COLMAP_NORMAL_CAMERA_AUTO": "Auto Estimate",
    "COLMAP_NORMAL_CAMERA_SCOPE_DEFAULT": "Whole Scene",
    "COLMAP_NORMAL_CAMERA_SCOPE_GROUP_FORMAT": "{source} / {size} / {count} images",
    "COLMAP_NORMAL_CAMERA_PARAMS_PLACEHOLDER": "fx,fy,cx,cy etc.",
    "COLMAP_NORMAL_CAMERA_APPLY": "Save",
    "COLMAP_NORMAL_CAMERA_PARAMS_INVALID": "Check the normal-image camera params: {value}\n{detail}",
    "COLMAP_NORMAL_CAMERA_MODEL_REQUIRED": "Choose a camera model when entering normal-image camera params.",
    ...
    "PHASE_METASHAPE_NERF": "Create Mixed Metashape Dataset",
    ...
    "TRAINING_TOOL_LICHTFELD_CARD_BODY": "Load a supported dataset and create a trained PLY with headless execution. Use this for long training runs without operating the app UI.",
    "TRAINING_TOOL_LICHTFELD_CARD_FOOTER": "Output: output/",
    ...
    "TRAINING_TOOL_POSTSHOT_CARD_BODY": "Pass the dataset to Postshot CLI, then create a Postshot project and run training. Use this when you want the result as a Postshot project.",
    "TRAINING_TOOL_POSTSHOT_CARD_FOOTER": "Output: output/*.psht",
    ...
    "STEP4_PIPELINE_DETAIL_SPHERESFM_OFF": "SphereSfM will not run this time. Click the circle button if you need it.",
...
TIPS: dict[str, str] = {
    ...
    "SFM_SELECT_ROUTE": "Choose an SfM route from the cards. COLMAP and SphereSfM are configured and run inside this step.",
    "SFM_BACK_TO_ROUTES": "Return to the SfM route cards.",
    "SFM_RUN_COLMAP": "Prepare Cubemap view images and run COLMAP/GLOMAP SfM.",
    "SFM_RUN_SPHERESFM": "Create a sparse model from 360 images with the SphereSfM COLMAP build.",
    ...
    "SFM_ROUTE_EXTERNAL": "Use camera results already prepared in Metashape, RealityScan, COLMAP, SphereSfM, or another SfM tool. Step 4 has nothing to run for this route.",
    "SFM_ROUTE_REALITYSCAN": "Convert source images and masks to Cubemap views, then write Metashape camera poses as RealityScan XMP. Point-cloud PLY is used only when needed.",
    ...
    "SFM_ROUTE_SPHERESFM": "The output is a SphereSfM model based on the COLMAP sparse format. Images are referenced as 360° images without Cubemap conversion.",
    ...
    "DATASET_TOOL_METASHAPE": "Use Metashape XML/PLY as the input and create a NeRF/3DGS-style dataset with transforms.json and pointcloud.ply.",
    "DATASET_TOOL_SPHERESFM": "Use a SphereSfM sparse model as the input and create a NeRF/3DGS-style dataset with transforms.json and pointcloud.ply. Run SfM itself in Step 4.",
    "DATASET_RUN_METASHAPE": "Create a training dataset from Metashape XML/PLY and the current output settings.",
    "DATASET_RUN_SPHERESFM": "Create a training dataset from a SphereSfM sparse model and the current output settings.",
    ...
    "RS_LFS_IMAGES": "Source image folder matching the image names in the CSV. Usually output/realityscan/images/ or the scene images/ folder.",
    "RS_LFS_MASKS": "Optional matching mask folder. White pixels are kept and black pixels are excluded.",
    "RS_LFS_OUTPUT": "Folder opened as a Dataset in LichtFeld. The tool creates images/, masks/, and sparse/0/ inside it.",
    "RS_LFS_PRE_UNDISTORT": "Enable this when a RealityScan realignment mixes Cubemap images with normal camera images. Only images with lens distortion coefficients are corrected into PINHOLE images so LichtFeld can train without special distortion handling. PINHOLE Cubemap images are referenced with Windows hard links. If distorted cameras remain while this is off, train in LichtFeld with GUT or Undistort enabled.",
    ...
    "ADD_INPUT_IMAGE_SEQUENCE": "Add a prepared still-image folder to the input source list. Images are copied into the scene images/ folder when the run starts.",
    ...
    "RUN_MASKS": "Run the primary mask generator and currently enabled extra mask steps for images selected by Scope. Use Missing only when you want existing masks left untouched.",
    ...
    "MASK_SCOPE": "Choose which images are processed. After adding sources in Step 1, use Missing only. When mask settings changed, use Missing + stale. Use Regenerate all only when you want to rebuild every mask",
    "MASK_SCOPE_MISSING": "Generate masks only for images that do not have a matching file in masks/. Existing and manually corrected masks are left untouched",
    "MASK_SCOPE_STALE": "Process missing masks and masks whose source image or mask settings changed since the last recorded generation. Masks that appear manually edited are protected",
    "MASK_SCOPE_ALL": "Rebuild masks for every image with the current settings. Existing and manually edited masks are overwritten",
    ...
    "EXPORT_METHOD": "Choose the dataset-step route: convert Metashape results, export COLMAP-view images, or run SphereSfM and convert its result.",
    ...
    "SPHERESFM_OUTPUT_SHAPE": "PINHOLE writes undistorted view images. ERP 360° uses 360° equirectangular images for LichtFeld GUT.",
    "SPHERESFM_TARGET_PROFILE": "Preset for the target app. ERP 360° output is fixed to LichtFeld.",
    ...
    "COLMAP_NORMAL_CAMERA": "Initial camera settings for normal images when they are mixed into COLMAP SfM with 360 images. Auto Estimate leaves initialization to COLMAP. Enter a model and params only when you have measured or calibrated intrinsics. Cubemap Rig images are not affected.",
    ...
    "OUTPUT_SHAPE": "PINHOLE writes undistorted view images. ERP 360° uses 360° equirectangular images for LichtFeld GUT.",

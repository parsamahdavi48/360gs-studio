...
STRINGS: dict[str, str] = {
    ...
    "DATASET_MASK_MODE_CONVERT_SFM": "Create from SfM Masks",
    "DATASET_MASK_MODE_GENERATE_TRAINING": "Regenerate Training Masks",
    "DATASET_MASK_MODE_REUSE_EXISTING": "Use Output Masks",
    "DATASET_MASK_MODE_NONE": "No Masks",
    "DATASET_MASK_NOTE": "Regenerate here when 3DGS training should use masks that differ from the SfM masks.",
    ...
    "DATASET_TOOL_SPHERESFM_DESC": "Create a 3DGS training dataset from a SphereSfM sparse model. PINHOLE output is written to output/spheresfm_cubemap/. ERP 360° output is written to output/spheresfm_3dgut/.",
    "DATASET_TOOL_SPHERESFM_CARD_BODY": "Convert a SphereSfM sparse model into a NeRF/3DGS-style JSON/PLY dataset.",
    "DATASET_TOOL_SPHERESFM_CARD_FOOTER": "Output: output/spheresfm_cubemap/ or output/spheresfm_3dgut/",
    ...
    "STEP4_TAB_MASK_SETTINGS": "Mask Settings",
...
TIPS: dict[str, str] = {
    ...
    "DATASET_MASK_MODE": "Choose how masks are written into the training dataset. Use the broad SfM masks, or regenerate masks specifically for 3DGS training.",
    "DATASET_MASK_MODE_CONVERT_SFM_MASKS": "Convert the Step 3 SfM masks into the current dataset format. Choose this when the broad SfM masks for sky, people, moving trees, and other unstable areas should also be used for training. This is the previous default behavior.",
    "DATASET_MASK_MODE_GENERATE_TRAINING": "Regenerate masks from the Step 5 output images. Choose this when SfM used broad masks, but 3DGS training should use a different mask target such as people only.",
    "DATASET_MASK_MODE_REUSE_EXISTING": "Use masks/ that already exists in the Step 5 output and update only mask_path entries in the transforms JSON. Choose this when masks were already created and you only want to change other dataset settings.",
    "DATASET_MASK_MODE_NONE": "Do not write mask_path entries into the training dataset transforms JSON. Existing masks/ files are not deleted. Choose this when training without masks or when masks are handled separately in the training app.",
    "DATASET_MASK_NOTE": "Step 5 mask settings are dataset-output specific. Step 3 masks/ remains the SfM mask source, while training masks are managed separately.",
    ...
    "DATASET_MASK_RUN_DATASET_ACTION": "SfM mask conversion runs as part of the dataset creation job.",
    ...
    "COLMAP_TEXT_MASK_MODE_GENERATE_TRAINING": "After creating output/metashape_colmap/images/, regenerate training masks from those images. Choose this when training masks should differ from the broad SfM masks.",
    ...
    "DATASET_EXPORT_TARGETS": "Choose whether to rewrite dataset images. Choose the training-mask behavior in the Mask Settings tab.",

...
4. Step 3で人物マスクを生成します。真上から見た撮影者が漏れる場合は `下部検出強化` を上げ、必要に応じてスティッチ境界、白飛びマスクも有効にします。
...
エクイレクタングラー360画像で底面の撮影者が漏れる場合は、Step 3の `下部検出強化` プリセットを使えます。底面再検出だけを対象に、YOLO信頼度の低下、底面画像の回転TTA、近傍フレームの時系列補完、必要に応じた底面専用YOLO Xを適用します。

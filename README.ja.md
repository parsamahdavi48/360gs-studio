...
- プレビューで漏れを見つけた場合は、設定を調整して `表示中を再生成` を使うと、その1枚だけ現在ONのマスク処理で `masks/` に保存し直せます。サムネイル一覧では `Ctrl` / `Shift` 選択した複数枚をまとめて再生成できます。SAM3.1では既存マスクへプロンプト検出結果を加算/減算して補正できます。
- `品質: 最高` はより重く、床や地面を余分にマスクする場合があります。

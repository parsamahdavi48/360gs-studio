...
4. Step 3で人物マスクを生成します。360°画像では `YOLO Level 2 高品質` が推奨開始点です。真上から見た撮影者が漏れる場合は `下部検出強化` を上げ、必要に応じてスティッチ境界、白飛びマスクも有効にします。
...
通常動画やデジタル一眼の連番画像を `images/` に置いた場合は、Step 3で `画像タイプ: 通常` を選ぶことで、スティッチ境界と360°底面再検出を使わずにYOLO/SAMや白飛びマスクだけを生成できます。

エクイレクタングラー360°画像で底面の撮影者が漏れる場合は、Step 3の `下部検出強化` プリセットを使えます。下部が十分にマスクされているなら標準、真上から見た撮影者が残る場合は `下部漏れ対策`、それでも残る場合だけ `下部漏れ対策・強` を使います。強くするほど処理時間が増え、床や地面を余分にマスクする場合があります。

...
- プレビューで漏れを見つけた場合は、設定を調整して `表示中を再処理` を使うと、その1枚だけ `masks/` に保存し直せます。サムネイル一覧では `Ctrl` / `Shift` 選択した複数枚をまとめて再処理できます。サムネイル表示中の矢印キーは、表示中のサムネイル選択として動作します。

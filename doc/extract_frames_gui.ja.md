...
Step 1 は、360°動画、または既に用意してある連番静止画フォルダから、SfM/3DGSに使うシーン画像を作る画面です。ここで作った `images/` と `_stechdrive/frames/selected_frames.csv` が、Step 2の確認、Step 3のマスク生成、MetashapeまたはSphereSfMへ進む土台になります。
...
2. `入力ソース` を選びます。動画から切り出す場合は `動画`、既に静止画がある場合は `画像フォルダ` を使います。
3. 動画モードでは、右側の `動画キュー` を確認します。シーンフォルダ内に動画があれば自動で登録されます。
4. 動画キューに目的の動画がない場合は、`動画を追加` で抽出する動画を追加します。追加操作は既存のキューを置き換えないため、別フォルダの動画も後から追加できます。
...
`入力ソース` が `画像フォルダ` の場合は、対応する静止画をシーンの `images/` に取り込み、動画抽出と同じ形式の確認メタデータを書きます。これにより、既存の連番画像も手作業でねじ込まずに Step 2 と Step 3 へ進めます。

...
`スケール` タブは書き出し後に使う後処理です。推定には作成済みの `<scene>/output/transforms.json` と `output/` 配下の画像が必要なため、下部の `実行` ボタンには含めていません。
...
4. `推定` を押します。この時点ではファイルを書き換えず、推定scaleと観測数などの統計だけを表示します。

...
左ナビゲーションには、Step 4のサブ工程として `SfM`、`Cubemap`、`Train` が常に表示されます。Step 4内では、ルートボタン `Metashape`、`COLMAP`、`SphereSfM` は `SfM` タブにあります。出力プリセット、画像/マスクのON/OFF、Cube6、Yaw、画像サイズは `Cubemap`、CLI実行の設定は `トレーニング` にあります。

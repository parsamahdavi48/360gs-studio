...
`トレーニング` タブでは、Step 4の書き出しやSfM変換が終わったあとに、外部CLIを続けて起動できます。上部で `LichtFeld`、`Postshot`、`その他...` を選び、`書き出し後にトレーニング開始` をONにします。`その他...` は `Custom` など追加候補だけを展開し、主要候補を混雑させないための選択です。

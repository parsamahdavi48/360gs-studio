...
`スケール` タブは書き出し後に使う後処理です。推定には作成済みの投影済みCubemap出力、つまり `<scene>/output/transforms.json` と `output/` 配下のピンホール画像が必要なため、下部の `実行` ボタンには含めていません。3DGUT向けのエクイレクタングラー出力のままでは推定せず、先にCubemap画像を書き出すよう案内します。

既知サイズで印刷したAprilTagをシーン内に置いて撮影した場合に使います。

1. 先にStep 4の出力を作成します。
2. `スケール` タブを開きます。
3. 印刷したタグの実寸、タグファミリ、撮影に使ったタグIDを入力します。別のタグを印刷していない限り、既定の `tag36h11 / ID 7` を使います。
4. `推定` を押します。処理中は下部ログと進捗バーに検出状況が表示されます。この時点ではファイルを書き換えず、推定scaleと観測数などの統計だけを表示します。
5. 結果が妥当な場合だけ `Scaleへ反映` を押します。現在のファイルをバックアップし、`output/transforms.json` のカメラ位置と、存在する場合は `output/pointcloud.ply` の点群座標へ同じscaleを掛けます。

折りたたみの `タグPDF` では、選択したタグをA4、A3、LetterのPDFとして出力できます。印刷時は100%/実寸で出力してください。プリンタ側で拡大縮小されると、推定に使うタグ実寸が変わります。

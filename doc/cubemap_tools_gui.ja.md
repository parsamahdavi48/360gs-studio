...
2. `スケール` タブを開きます。
3. 印刷したタグの実寸、タグファミリ、撮影に使ったタグIDを入力します。別のタグを印刷していない限り、既定の `tag36h11 / ID 7` を使います。`出力プリセット` は通常 `自動判定` のままにし、Step 4設定がない出力で作成時のCubemapルールが分かっている場合だけ明示指定します。
...
`LichtFeld Studio` プロファイルでは、Cubemapの `transforms.json` を `PINHOLE` として出力したあと、LichtFeld向けの最終向き補正を `transforms.json` と `pointcloud.ply` の両方に適用します。この2つはセットで扱い、片方だけを回転したり差し替えたりしないでください。

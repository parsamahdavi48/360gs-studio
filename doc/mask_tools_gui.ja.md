...
- `YOLO Level`:
  ...
  - 360°画像では `2 高品質・360°推奨` から始めるのを推奨します。
  - 処理時間を優先する確認用は `1 標準`、それでも人物が漏れる場合は `3 最高・重い` を使います。
  - 通常画像では `1 標準` から始めます。
- `YOLO Expand (px)`:
  - `yolo_mask.py --expand`
...
- `下部検出強化`:
  ...
  - `下部漏れ対策`: 真上から見た撮影者、三脚、手元が下部に残る素材向け。まずはこちらを試します。
  - `下部漏れ対策・強`: `下部漏れ対策` でも下部に人物が残る場合のみ使います。より重く、床や地面を余分にマスクしやすくなります。
  - `画像タイプ=通常` では使いません。
- `YOLO Classes`:
...
- 下部の撮影者だけ漏れる: `下部検出強化 標準 -> 下部漏れ対策`。それでも足りない場合は `下部漏れ対策・強`
